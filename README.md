# Sakila DVD Rental: Power BI Dashboard

A two-page Power BI dashboard built on the Sakila DVD rental sample database, analyzing revenue, customer value, and inventory efficiency. Findings were cross-validated against SQL before being trusted in any visual.
...
This project analyzes the [Sakila database](https://github.com/bradleygrant/sakila-sqlite3), a sample relational database modeling a fictional DVD rental store covering films, categories, inventory, customers, rentals, and countries. As with this portfolio's other relational projects, the analysis required joining across multiple tables rather than working from a single flat file.
...
2. **Export to CSV**: tables were exported from SQLite and re-imported into Power BI. This is a deliberate simplification for a portfolio project, with the trade-off (no live refresh from the source database) noted rather than hidden.
...
5. **Deliberate handling of orphaned data**: 5 payment records exist with null `rental_id` values (possibly fees or adjustments). These were included in KPI card totals as real revenue but excluded from category-level visuals, where no meaningful category could be assigned. This is a deliberate decision rather than a silent omission.
## Key findings
...
- **Underperforming titles are spread across genres**, not concentrated in one category. Inventory inefficiency is not a single-category problem.
...
- **Top customers vary meaningfully in both rental frequency and spend efficiency.** Unlike Chinook (where every top-10 customer had an identical order count of 7), Sakila's top customers range from 34 to 46 rentals. ANA BRADLEY has the highest average revenue per rental ($5.14) despite ranking 10th by total spend, suggesting she consistently picks higher-priced films rather than simply renting more often.
- **A real data-quality issue surfaced and was kept, not hidden.** "Runion" in the Top Customers table reflects a character-encoding issue for Reunion (a French island territory) in the source database. It is flagged directly on the dashboard rather than silently corrected.
## A note on scope
...
- Power BI connects to CSV exports of the Sakila tables rather than a live ODBC connection to the SQLite database. This is a deliberate simplification that means the dashboard does not refresh dynamically from the source.
## Files in this repo
...
- Correct the Reunion encoding issue at the source and re-validate the Top Customers table
- Investigate what actually drives the three-tier pricing structure. The schema does not capture it, but a real store's pricing logic presumably exists somewhere
